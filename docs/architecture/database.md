...
|---|---|---|---|---|
| A | 品目ID(itemId) | 文字列 | ○ | 一意。自社発行QRの場合は`PART-000123`形式、メーカー添付バーコードから登録した場合はGTIN(例: `04901234567894`)を用いる(要件定義FR-12、全体設計書.md 4.5節参照) |
| B | 品目名(itemName) | 文字列 | ○ | |
| C | 現在在庫数(currentStock) | 数値 | ○ | 0以上 |
| D | 閾値(threshold) | 数値 | ○ | 0以上 |
| E | アラート送信済みフラグ(alertSentFlag) | 真偽値(TRUE/FALSE) | ○ | 初期値FALSE |
| F | 保管場所(location) | 文字列 | 任意 | |
| G | 廃番フラグ(discontinuedFlag) | 真偽値 | ○ | 初期値FALSE。管理者のみ変更可(FR-11) |
| H | 更新年月日(updatedAt) | 日時 | ○ | 更新の都度自動更新 |

> 管理者のDB直接編集とLockServiceの競合リスクあり、整備中の直接編集は避け、もし問題があれば棚卸で解消する

## 2. 入出庫履歴シート（`TransactionLog`）

| 列 | 項目名(フィールド名) | 型 | 必須 | 備考 |
|---|---|---|---|---|
| A | 入出庫日時(transactionAt) | 日時 | ○ | サーバー側で自動付与 |
| B | 品目ID(itemId) | 文字列 | ○ | 在庫マスタと紐付け |
| C | 種別(type) | 文字列 | ○ | `IN` または `OUT` |
| D | 数量(quantity) | 数値 | ○ | 0以上 |
| E | ユーザ識別(userEmail) | 文字列 | ○ | ログイン中のメールアドレス |
...
| 列 | 項目名(フィールド名) | 型 | 必須 | 備考 |
|---|---|---|---|---|
| A | 通知先ID(targetId) | 文字列 | ○ | 一意 |
| B | 通知先メールアドレス(email) | 文字列 | ○ | |
...
| C | 通知先ID(targetId) | 文字列 | 条件付 | 管理者は通知先設定に対応するIDを設定、非管理者は空白 |
| D | 退職フラグ(retiredFlag) | 真偽値 | ○ | trueの場合ログイン拒否 |
| E | 更新年月日(updatedAt) | 日時 | ○ | |

> 「管理者級の整備士」の判定は、C列（通知先ID）が空欄でないことで行う \

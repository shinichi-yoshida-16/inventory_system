...
### 1.3 sessionStorageが利用できない場合の挙動(2026-08-20追記)

- 事象：ログインボタン押下後、エラーメッセージも表示されずGF-00(ログイン画面)に留まったまま反応がないという報告があった
- 原因：プライベートブラウズモードやアプリ内ブラウザ等でsessionStorageの読み書きがブロックされている環境では、`login()`(GAS側)自体は`status:"OK"`で正常終了するが、クライアント側の`saveSession()`(sessionStorage.setItem)が例外を投げ、後続のGF-01への画面遷移が実行されずに処理が止まっていた。呼び出し元でこの例外を捕捉していなかったため、利用者にはエラー表示すら出ない「無反応」に見えていた
- 対応（GF-00のみ）：
  1. ページ読み込み時に`isSessionStorageAvailable_()`(session.html)でストレージ利用可否を事前チェックし、利用不可の場合は警告メッセージを表示する(ログイン試行自体は妨げない)
  2. `doLogin()`成功時の`saveSession()`＋画面遷移処理を`try/catch`で保護し、失敗時は原因が分かるメッセージを表示する
- 残課題：本設計はセッション維持をsessionStorage前提としており(1.1〜1.2節)、ストレージが完全にブロックされた環境そのものを救済する代替手段は持たない。恒久対応の検討状況は`全体設計書.md` 8.2節を参照
